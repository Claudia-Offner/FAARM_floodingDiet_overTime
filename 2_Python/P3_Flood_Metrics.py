...
sub = sub[['c_code', 'year_season', 'year', 'season', 'month', 'c_area', 'c_floodedAreakm2',
           'r_area', 'r_floodedAreakm2', 'perc_flooded_c']]
# sub['r_perc_flooded_c'] = sub['r_floodedAreakm2'] / sub['r_area']

# Statistics over time (for all c_codes)
ystat_c = Statistics(sub).stats_by_time('perc_flooded_c', 'year', range(2015, 2020, 1))
ystat_c = ystat_c.add_prefix('avYearly_').rename(columns={'avYearly_year': 'year'})
sstat_c = Statistics(sub).stats_by_time('perc_flooded_c', 'season', range(1, 7, 1))
sstat_c = sstat_c.add_prefix('avSeason_').rename(columns={'avSeason_season': 'season'})

# Statistics over time (for Sylhet division)
# ystat_r = Statistics(sub).stats_by_time('r_perc_flooded_c', 'year', range(2015, 2020, 1))
# ystat_r = ystat_r.add_prefix('avYearly_').rename(columns={'avYearly_year': 'year'})
# sstat_r = Statistics(sub).stats_by_time('r_perc_flooded_c', 'season', range(1, 7, 1))
...
# mstat = Statistics(sub).stats_by_time('perc_flooded_c', 'month', range(1, 13, 1))
...
# div = seasonal_weight(div, 'r_perc_flooded_c')  # Weight
# div['diff'] = div['perc_flooded_c']-div['avSeason_mean']  # Anomaly
# # Visualise
# visual(div, 'perc_flooded_c', 'Division Level')
...
all_clust = seasonal_weight(all_clust, 'perc_flooded_c')  # Weight
all_clust['diff'] = all_clust['perc_flooded_c']-all_clust['avSeason_mean']  # Anomaly
# Visualise
visual(all_clust, 'perc_flooded_c', 'Cluster Level (All)')

# Individual Cluster Average
c = sub.groupby(['c_code', 'season'], as_index=False).mean()
# Average by cluster and season
c = c[['c_code', 'season', 'perc_flooded_c']].rename(columns={'perc_flooded_c': 'avSeason_mean'})
...
sing_clust = seasonal_weight(sing_clust, 'perc_flooded_c')  # Weight
sing_clust['diff'] = sing_clust['perc_flooded_c']-sing_clust['avSeason_mean']  # Anomaly
# Visualse individual cluster
sing_clust = sing_clust[sing_clust['c_code'] == 43]
visual(sing_clust, 'perc_flooded_c', 'Single Cluster Level')
...
y = df[['c_code', 'year_season', 'perc_flooded_c']].drop_duplicates(subset=['c_code', 'year_season'], keep='last').reset_index(drop=True)
groups = y.groupby('c_code')
y['flooded_diff'] = ''
for cluster, group in groups:
    # Flood weight
    group['diff'] = np.insert(np.diff(group['perc_flooded_c']), 0, 0)
...
RESULT = pd.merge(RESULT, y[['c_code', 'year_season', 'flooded_diff']], how='left', on=['c_code', 'year_season'])  # Difference
RESULT = seasonal_weight(RESULT, 'perc_flooded_c')  # Weight
RESULT['flooded_anom'] = RESULT['perc_flooded_c']-RESULT['avSeason_mean']  # Anomaly
