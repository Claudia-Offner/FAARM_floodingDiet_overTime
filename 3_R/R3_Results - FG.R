...
formula <- as.formula(paste0('dd10r_min_m', interaction, " + dd10r_score_m_BL + ramadan + g_2h_BL + quint2_BL + (1 | wcode) + (1 | c_code) + (1 | season_id)"))
...
extract_GLMER <- function(variable, folder) {
  ...
  print(paste0("VARIABLE ", variable, " IS COMPLETE"))
...
library(glmmTMB)
glmmTMB_simple_fit <- glmmTMB( dd10r_min_m ~ Flood_1Lag + (1|wcode) + ar1(season_id+0|wcode), data=df,family=binomial)
...
plotResults(mod_res, 1)

# MARGINAL EFFECTS
ame1 <- emtrends(glmm_mod, pairwise ~ Flood_1Lag*season_flood, var = 1)$emtrends
...
emm_2 <- emmeans(glmm_mod, ~ Flood_1Lag*season_flood, at = list(Flood_1Lag = levels)) # levels
