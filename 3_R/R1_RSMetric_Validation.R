...
# T TESTS
t.test(df$flood33_bin, df$perc_flooded_cub)
t.test(df$flood234, df$perc_flooded_cub)
t.test(df$flood231, df$perc_flooded_cub)
t.test(df$dd10r_min_m, df$perc_flooded_cub)
## Lagged flooding
t.test(df$flood33_bin, df$Flood_1Lag_cub)
t.test(df$flood234, df$Flood_1Lag_cub)
t.test(df$flood231, df$Flood_1Lag_cub) # not sig
t.test(df$dd10r_min_m, df$Flood_1Lag_cub)

# GENERAL LINEAR MODELS
#### COPING STRATEGY
bme <- glm(flood231 ~ perc_flooded_cub, family='binomial', data=df)
summary(bme) 

# Strong positive association: increases in flooding give an 8.18 odds in developing coping strategies 

####  BOROWING MONEY/FOOD
bme <- glm(flood234 ~ perc_flooded_cub, family='binomial', data=df)
summary(bme) 

# Some evidence for positive association: increases in flooding  give 4.2 odds in borrowing money/food to coping strategies 

####  FLOOD PERCEPTION
bme <- glm(flood33_bin ~ perc_flooded_cub, family='binomial', data=df)
summary(bme) 
...
x <- glm(dd10r_score_m ~ Flood_1Lag_cub+treatment+wealth2_BL+dd10r_score_m_BL, data=df)
summary(x)
...
x <- glm(dd10r_score_m_EL ~ Flood_1Lag_cub+treatment+wealth2_BL+dd10r_score_m_BL, data=df)
summary(x)
...
RS_metric <- inla(dd10r_score_m ~ Flood_1Lag_cub+treatment+wealth2_BL+dd10r_score_m_BL+
