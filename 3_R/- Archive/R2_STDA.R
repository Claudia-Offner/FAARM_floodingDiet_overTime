...
ggplot(df,aes(x=season_flood2,y=perc_flooded_c))+
...
(mean_by_category <- tapply(df$perc_flooded_c, df$season_flood2, mean, na.rm = TRUE))
...
exp_time <-cast(df, c_code~year, mean, value = "perc_flooded_c") # in years
...
exp_time_ys<-cast(df, c_code~year_season, mean, value = "perc_flooded_c") # in years
s <- subset(df, select = c("c_code", "lat", "long"))
s <- s[!duplicated(s), ] # remove duplicates
...
ggplot(df) +
  geom_tile(aes(x=year_season, y=reorder(c_code, wcode, mean, order=TRUE), fill = perc_flooded_c)) +
...
ggplot(df) +
  geom_tile(aes(x=year_season, y=reorder(c_code, lat, mean, order=TRUE), fill = perc_flooded_c)) +
  scale_fill_gradient(name = "% Flooded",low = "blue", high = "red") +
  labs(x = "Year-Season", y = "Cluster Code")
...
# Raw
crude_gme <- lmer(dd10r_score_m ~ perc_flooded_c+(1|wcode),
                  data=df)
